(function(){
  ...
  const DEFAULT={profile:{name:"JUGADOR",number:1,color:"#2d9cff",skin:"rookie"},coins:80,unlocked:["rookie"],stats:{wins:0,games:0,trophies:0,streak:0},tournament:{lastStarted:0,bestRound:0,current:null},settings:{serverUrl:""}};
  ...
  function startTournament(state){data.tournament.lastStarted=Date.now();data.tournament.current=clone(state);save();}
  function saveTournament(state){data.tournament.current=clone(state);data.tournament.bestRound=Math.max(data.tournament.bestRound||0,(state.round||0)+1);save();}
  function winTournament(state){if(!state.rewarded){data.stats.trophies++;state.rewarded=true;}data.tournament.bestRound=4;data.tournament.current=clone(state);save();}
  window.BasketStore={get:()=>data,save,profile,addCoins,recordGame,buy,tournamentAvailable,startTournament,saveTournament,winTournament,SHOP};
})();
