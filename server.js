...
io.on("connection",socket=>{
  ...
  socket.on("input",payload=>{const room=roomOf(socket);if(!room?.started||room.closed||payload?.action!=="jump")return;const now=Date.now();if(now-socket.data.lastInput<55)return;socket.data.lastInput=now;const team=room.players.indexOf(socket.id);if(team<0||team>1)return;room.game.input(team);});
